...
class World:
    ...
    def _create_submatrix_histogram(self, chunks):
        ...
        for x in range(0, len(chunks)):
            ...
            temp = self._find_largest_rect(histogram)  # Find largest rectangle in region cluster
            if (temp[0] * temp[1] > max_dimensions[0] * max_dimensions[1]):
                max_dimensions = temp
    ...
    def _find_largest_rect(self, histogram):
        ...
        for i in range(0, len(histogram)):
            ...
            if not p_stack or h > h_stack[-1]:
                h_stack.append(h)
                p_stack.append(i)
            elif h < h_stack[-1]:
                while not h_stack and h < h_stack[-1]:
                    temp_h = h_stack.pop()
                    temp_pos = p_stack.pop()
                    temp_size = temp_h * (i - temp_pos)
                    max_size = max(temp_size, max_size)

                    if max_size == temp_size:
                        dimensions = (i - temp_pos, temp_h)

                h_stack.append(h)
                p_stack.append(temp_pos)
